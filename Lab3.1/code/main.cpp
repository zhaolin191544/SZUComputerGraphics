...
void display_3()
{
	...
	float left = -camera_3->scale;
	float right = camera_3->scale;
	float bottom = -camera_3->scale;
	float top = camera_3->scale;
    camera_3->projMatrix = camera_3->ortho(left, right, bottom, top, camera_3->zNear, camera_3->zFar);
...
{	
	...
	GLFWwindow* mainwindow = glfwCreateWindow(WIDTH, HEIGHT, "2022220036_赵麟_实验3.1", NULL, NULL);
	if (mainwindow == NULL)
	{
