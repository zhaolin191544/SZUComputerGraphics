...
void init()
{
	...
	table->setScale(glm::vec3(1.8, 1.8, 1.8));
	...
	wawa->setRotation(glm::vec3(-90, 0.0, 0.0));
	wawa->setScale(glm::vec3(1.8, 1.8, 1.8));

	// 设置材质
	wawa->setAmbient(glm::vec4(0.2, 0.2, 0.2, 1.0));
	wawa->setDiffuse(glm::vec4(0.8, 0.8, 0.8, 1.0));
	wawa->setSpecular(glm::vec4(0.2, 0.2, 0.2, 1.0));
	wawa->setShininess(2.0);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	float tmp;
	glm::vec4 ambient;
	if (action == GLFW_PRESS || action == GLFW_REPEAT) {  // 添加GLFW_REPEAT以支持按键持续
		switch (key)
		{
		...
		default:
			...
			camera->updateCamera(); // 在每次键盘输入后更新相机
			break;
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "2022220036_赵麟_实验4", NULL, NULL);
	if (window == NULL)
	{
