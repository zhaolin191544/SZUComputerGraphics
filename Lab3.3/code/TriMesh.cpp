...
void TriMesh::computeTriangleNormals()
{
	...
	for (size_t i = 0; i < faces.size(); i++) {
		auto& face = faces[i];
		glm::vec3 v1 = vertex_positions[face.y] - vertex_positions[face.x];
		glm::vec3 v2 = vertex_positions[face.z] - vertex_positions[face.x];
		glm::vec3 norm = glm::normalize(glm::cross(v1, v2)); // 叉积得到法向量并归一化
		face_normals[i] = norm;
	}
}
