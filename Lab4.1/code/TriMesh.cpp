...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	vertex_normals.push_back(glm::vec3(0.0, 0.0, 1.0));
	vertex_colors.push_back(glm::vec3(0.0, 0.0, 1.0));
	...
	// 生成三角形面片
	for (int i = 0; i < num_samples; i++) {
		faces.push_back(vec3i(0, i + 1, ((i + 1) % num_samples) + 1));
		
		// 添加纹理坐标
		vertex_textures.push_back(glm::vec2(0.5, 0.5));
		vertex_textures.push_back(glm::vec2(0.5 + 0.5*cos(i*step), 0.5 + 0.5*sin(i*step)));
		vertex_textures.push_back(glm::vec2(0.5 + 0.5*cos((i+1)*step), 0.5 + 0.5*sin((i+1)*step)));
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
    ...
    // 修改顶点位置：将顶点放在(0,height,0)而不是(0,0,height)
    vertex_positions.push_back(glm::vec3(0.0, height, 0.0));
    vertex_normals.push_back(normalize(glm::vec3(0.0, 1.0, 0.0)));
    vertex_colors.push_back(normalize(glm::vec3(0.0, 1.0, 0.0)));

    // 修改底面圆周上的顶点：在xz平面上生成
    for (int i = 0; i < num_samples; i++) {
        float theta = i * step;
        float x = radius * cos(theta);
        float z = radius * sin(theta); // 使用z而不是y
        vertex_positions.push_back(glm::vec3(x, 0.0, z));
        
        // 计算法向量(需要考虑斜面)
        glm::vec3 normal = normalize(glm::vec3(x, -height, z));
        vertex_normals.push_back(normal);
        vertex_colors.push_back(normal);
    }
